        ...
        private void AskName(string userInput)
        {
            if (string.IsNullOrEmpty(userName))
            {
                ...
                if (!string.IsNullOrWhiteSpace(userInput) && !userInput.ToLower().Contains("my name is"))
                {
                    ...
                    // Wait for next input; we'll store the name on the next call.
                    // But we need to capture the next message as name.
                    // Simple approach: set a flag. We'll store name when ProcessInput is called again.
                    // We'll store userInput as name if we are in "waiting for name" state.
                    // For simplicity, assume the user replies with just their name.
                    // We'll add a state variable.
                }
                else
                {
                    // Extract name from "My name is John"
